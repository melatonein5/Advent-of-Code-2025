...
The first character will always be the direction of the rotation (`R` or `L`). I am storing this as a boolean, with `R` being represented by `true` and `L`  being representing by `false` (in fact, anything that isn't an `R` is represented by `false`, which leads to behaviour where `☭345` is a left turn by 345, and I suppose it is appropriate that a hammer and sickle glyph results in a rotation to the left).
